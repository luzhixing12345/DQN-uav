...
class UAV:
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                for k in range(-1, 2):
                    ...
                    if (
                        self.x + i < 0
                        or self.x + i >= self.ev.len
                        or self.y + j < 0
                        or self.y + j >= self.ev.width
                        or self.z + k < 0
                        or self.z + k >= self.ev.h
                    ):
                        self.ob_space.append(1)
                        state_grid.append(1)
                    else:
                        self.ob_space.append(
                            self.ev.map[self.x + i, self.y + j, self.z + k]
                        )  # 添加无人机临近各个栅格状态
    ...
    def update(self, action):
        ...
        if self.nearest_distance >= 4 or self.ev.WindField[0] <= self.ev.v0:
            ...
            self.p_crash = math.exp(
                ...
                * self.ev.v0
                / (
                    0.5
                    * math.pow(self.ev.WindField[0] * math.cos(abs(self.ev.WindField[1] - self.dir_ob) - self.ev.v0), 2)
                )
            )
            # self.p_crash=0
        ...
        if (
            self.x <= 0
            or self.x >= self.ev.len - 1
            or self.y <= 0
            or self.y >= self.ev.width - 1
            or self.z <= 0
            or self.z >= self.ev.h - 1
